...
public class ArchiveInfo implements Serializable {
	...
	public ArchiveInfo(Info info) {
		this.id = info.getId();
		
		this.sonDepartmentId = info.getSonDepartmentId();
		
		this.sonDepartmentName = info.getSonDepartmentName();

		this.personGroup = info.getPersonGroup();

		this.personName = info.getPersonName();

		this.personSex = info.getPersonSex();

		this.personNation = info.getPersonNation();

		this.personId = info.getPersonId();

		this.personImage = info.getPersonImage();

		this.personAddr = info.getPersonAddr();

		this.personNum = info.getPersonNum();

		this.personTel = info.getPersonTel();

		this.houseAge = info.getHouseAge();

		this.houseOldType = info.getHouseOldType();

		this.houseOldSize1 = info.getHouseOldSize1();

		this.houseOldSize2 = info.getHouseOldSize2();
		
		this.toiletType = info.getToiletType();

		this.toiletTypeName = info.getToiletTypeName();

		this.buildMode = info.getBuildMode();

		this.buildModeName = info.getBuildModeName();

		this.rebuildMode = info.getRebuildMode();

		this.rebuildModeName = info.getRebuildModeName();

		this.buildCompany = info.getBuildCompany();
		
		this.houseNewSize1 = info.getHouseNewSize1();

		this.houseNewSize2 = info.getHouseNewSize2();
		
		this.planYear = info.getPlanYear();

		this.rebuildBeginDate = info.getRebuildBeginDate();
		...
		this.isAcceptance = info.getIsAcceptance();

		this.grantType = info.getGrantType();

		this.grantTypeName = info.getGrantTypeName();
		
		this.sumFund = info.getSumFund();

		this.grantProvinceFund = info.getGrantProvinceFund();

		this.grantCountiesFund = info.getGrantCountiesFund();
		
		this.personSelfFund = info.getPersonSelfFund();

		this.houseOldImage = info.getHouseOldImage();

		this.houseBuildingImage = info.getHouseBuildingImage();

		this.houseOutNewImage = info.getHouseOutNewImage();

		this.houseInNewImage = info.getHouseInNewImage();
		
		this.personDelegateImage = info.getPersonDelegateImage();
		
		this.fillUserName = info.getFillUserName();
		
		this.fillUserTel = info.getFillUserTel();
		
		this.fillUserUnit = info.getFillUserUnit();
		
		this.auditDepartmentId = info.getAuditDepartmentId();
